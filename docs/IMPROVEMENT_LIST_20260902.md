...
| F4 | shake 平滑 | 白噪 → 阻尼正弦 | 高 amp 不再雪花（目检） | ✅ 43d7730 |
| F5 | 载具转向曲线统一 | vehicle/moto 线性 → 马方案 pow(1.5) 指数 | 高速转向率明显低于低速（数据可测） | ✅ 43d7730 |
| F6 | 摩托引擎音 | 搬 vehicle 引擎块，pitch .85→1.65 | 骑乘可闻 RPM 随速 | ✅ 43d7730 |
| F7 | 木筏惯性 | move_toward 4.0 → lerpf(6→3, ratio)+回弹 | 松手滑行减速非瞬停（--ridertest 位移曲线） | ✅ 43d7730 |
...
| A2 | 蹄声独立 | horse 用 footstep_sand pitch 0.75 替代 heavy_impact 复用作蹄 | 疾驰音与重击音不同采样（听感+日志音名） | ✅ 43d7730 |
| A3 | 命中层优先级 | hit/headshot 独立小池防枪声抢占 | 8 枪并发时 hit 不丢（日志计数） | ✅ 43d7730 |
