...
> **免责声明 / 原创声明**：本项目为原创程序化实现，所有地形、模型、材质、音效均为代码生成，不含任何外部游戏的美术/音频/商标资产。文中“战地式”“旷野之息风格”“海拉鲁阔野”等仅为玩法灵感的通用表述，与任何商业 IP 无关联，不代表官方或授权关系。
...
- 神庙试炼：四座神庙各藏差异化限时试炼（符文/火盆/压力板/推球入臼，限时 15/18/20s，奖励 orb/seed 分级），射中全部 4 个符文获得精灵宝珠（生命上限 +10），超时重置可反复挑战
...
- 林克时间：Q 闪身带无敌帧，窗口内被击中触发 0.22 倍慢动作并无伤反击；河里可抓鱼，石头阵呀哈哈站进缺口补全出种子
- 莫布林：大体型重击敌人，长前摇举棒（可格挡可闪避）后猛击；烤串（兽肉+蘑菇）90 秒攻击 +25%
- 猎弓：按住左键拉弦蓄力，松开放箭，箭速与伤害随蓄力提升，箭矢走抛物线；遗迹与高原古树各有一把
- 林克之家：高原小屋里的床铺，按 E 睡到天亮并回满生命与精力；蜥蜴战士环绕游走并高速突进
- 弹反：举盾面向来袭石头可将其原路弹回；每收集 3 颗海拉鲁种子精力上限 +10
- 光束弹反：完美格挡守卫光束可直接摧毁守卫；桥西码头有木筏可乘（F 上下），接近巨龙时显示血条
- NPC 任务链：驿站老板收 3 个蘑菇（奖护甲）、城堡卫兵讨 2 只莫布林（奖宝珠），HUD 实时显示任务进度；花径追踪呀哈哈
- 新增任务：火山研究员收龙鳞（奖精力上限）、护送行商走到桥头（奖弹药）；焚天者半血暴怒进入第二阶段
- 血月苏醒覆盖全部敌人类型；集齐 10 颗种子获得呀哈哈面具（永久攻击 +5%）
...
- 合规文档见 `docs/LICENSE` / `docs/PRIVACY.md` / `docs/EULA.md`。
- 本作为单机离线体验（见 `scripts/main.gd` 顶部注释），二周目通过 `--seed` 随机种子池验证复玩增量（C7）。
