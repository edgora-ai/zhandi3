	...
	if _ap:
		if _buck_t > 0.0:
			...
			if _gallop_cd <= 0.0:
				...
				if _sfx_hg:
					_sfx_hg.play_at("footstep_hoof", global_position, -10.0, randf_range(0.95, 1.1))  # // FIX: HOOF 蹄声独立
			...
			if _gallop_cd <= 0.0:
				...
				if _sfx_ht:
					_sfx_ht.play_at("footstep_hoof", global_position, -12.0, randf_range(0.9, 1.0))  # // FIX: HOOF
